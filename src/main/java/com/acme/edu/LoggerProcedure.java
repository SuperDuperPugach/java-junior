...
public class LoggerProcedure {
    ...
    public static void log(int message) {
        ...
        LoggerProcedure.summ += message;
        if(!LoggerProcedure.isSumm)
            LoggerProcedure.isSumm = true;
    }
    ...
    public static void log(String message) {
        closeInt();
        if(message.equals(LoggerProcedure.prevStr)) {
            LoggerProcedure.strCount++;
        }
        else if(LoggerProcedure.strCount != 0) {
            closeStr();
            strFirstSet(message);
        } else {
            strFirstSet(message);
    ...
    private static void closeInt() {
       if(LoggerProcedure.isSumm) {
           print(PRIMITIVE_FORMAT, Integer.toString(LoggerProcedure.summ));
           LoggerProcedure.isSumm = false;
           LoggerProcedure.summ = 0;
       }
    }
    ...
    private static void closeStr() {
        if(LoggerProcedure.strCount == 0) return; //нечего выводить
        if(LoggerProcedure.strCount == 1) {
            print(STRING_FORMAT, LoggerProcedure.prevStr);
        } else {
            print(STRING_FORMAT_REPEAT, LoggerProcedure.prevStr, Integer.toString(LoggerProcedure.strCount));
        }
        strReset();
    }
    ...
    private static void strReset() {
        LoggerProcedure.strCount = 0;
        LoggerProcedure.prevStr = "";
    }
    ...
    private static void strFirstSet(String message) {
        LoggerProcedure.strCount = 1;
        LoggerProcedure.prevStr = message;
    }
    ...
    private static boolean isIntOverFlow(int message) {
        return ((long) LoggerProcedure.summ + (long)message) > Integer.MAX_VALUE ||
                ((long) LoggerProcedure.summ + (long)message) < Integer.MIN_VALUE;
    }
